  ...
  async function handleReset() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessage({
          type: 'success',
          text: `Contraseña actualizada para ${data.email ?? tenantName}.`,
        });
        setPassword('');
      }
    } catch {
  ...
  return (
    <div className="rounded-xl border border-slate-200/60 bg-white p-5">
      <div className="mb-4 flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-slate-900">Contraseña del cliente</h3>
      </div>

      <p className="mb-3 text-xs text-slate-500">
        Establece una nueva contraseña para el usuario del portal de {tenantName}.
      </p>
